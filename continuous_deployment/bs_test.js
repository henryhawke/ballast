...
module.exports.runTest = async (driver) => {
  try {
    await driver.get("https://www.ballast.web.app");

    await sleep(2000);

    var signInButton = driver.wait(
      webdriver.until.elementLocated(webdriver.By.name("signin"))
    );
    ...
    var emailInput = driver.wait(
      webdriver.until.elementLocated(webdriver.By.name("email"))
    );
